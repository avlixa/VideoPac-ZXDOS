...
// Valores iniciales menu
	...
	while(1)
	{
		...
		if(joy_pins & 0x100) { //ZXUNO
			dipsw=(MENU_CYCLE_VALUE(&topmenu1[6])<<4);	// (1bit: 4)Take the value of NTSC/PAL mode
			dipsw|=(MENU_CYCLE_VALUE(&topmenu1[7])<<5);	// (2bit: 5:6)Take the value of the color mode
			dipsw|=(MENU_CYCLE_VALUE(&topmenu1[8])<<2);	// (2bit: 2:3)Take the value of the board model
	  }
		else { //ZXDOS
			dipsw=(MENU_CYCLE_VALUE(&topmenu[6])<<4);	// (1bit: 1)Take the value of NTSC/PAL mode
			dipsw|=(MENU_CYCLE_VALUE(&topmenu[7])<<5);	// (2bit: 5:6)Take the value of the color mode
